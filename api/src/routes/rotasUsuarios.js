...
router.get('/', verificarToken, async (req, res) => {
    try {
        ...
        return res.status(500).json({ 
            error: "InternalServerError: Erro na instrução SELECT ao listar registros da tabela 'usuarios'. Motivo técnico: " + error.message,
...
// ==========================================
// 📌 LOGIN (AUTENTICAR)
// URL Externa: POST /auth/login (Mapear no server como /auth)
// ==========================================
    ...
    try {
        ...
        if (rowCount === 0) {
            return res.status(401).json({ 
                error: "InvalidCredentialsError: Nenhuma correspondência encontrada na tabela 'usuarios' para o email '" + email + "' combinado com o tipo de usuário '" + tipo_usuario + "'.",
        ...
        if (usuario.senha !== senha) {
            return res.status(401).json({ 
                error: "InvalidCredentialsError: O email '" + email + "' existe, mas a senha enviada no body não confere com a senha armazenada no banco de dados.",
                message: "Senha incorreta. Tente novamente." 
            });
        }

        // 💡 Injetando os dados fundamentais para a rota /perfil coletar automaticamente
...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        return res.status(500).json({
            error: "InternalServerError: Falha ao tentar rodar a instrução INSERT na tabela 'usuarios'. Motivo técnico: " + error.message,
    ...
    try {
        ...
        if (rowCount === 0) {
            return res.status(404).json({ 
                error: "ResourceNotFound: A instrução UPDATE executou com sucesso, mas afetou 0 linhas. Motivo: O ID '" + id + "' enviado no parâmetro da URL não foi localizado na tabela 'usuarios'.",
        ...
        return res.status(500).json({ 
            error: "InternalServerError: Falha no processamento da cláusula UPDATE para o ID '" + id + "' na tabela 'usuarios'. Motivo técnico: " + error.message,
...
// URL Externa: DELETE /usuarios/:id
// ==========================================
router.delete('/:id', verificarToken, async (req, res) => {
    ...
    try {
        ...
        if (rowCount === 0) {
            return res.status(404).json({ 
                error: "ResourceNotFound: O comando DELETE foi enviado, mas nenhuma linha foi removida. Motivo: O registro com o ID '" + id + "' não existe na tabela 'usuarios'.",
        ...
        return res.status(500).json({ 
            error: "InternalServerError: Erro crítico ao tentar remover o ID '" + id + "' da tabela 'usuarios'. Motivo técnico: " + error.message,
